...
sapply(paste0("seir/scripts/", f), source)
...
write_tsv(pred_clean, paste0("seir/data/prediction_", state, "_", format(max_date, "%Y%m%d"), ".txt"))
write_tsv(as_tibble(result$mcmc_pars, .name_repair = "unique"), paste0("seir/data/prediction_pars_", state, "_", format(max_date, "%Y%m%d"),".txt"))

write_tsv(pred_clean, paste0("seir/data/prediction_", state, "_latest.txt"))
write_tsv(as_tibble(result$mcmc_pars, .name_repair = "unique"), paste0("seir/data/prediction_pars_", state, "_latest.txt"))
...
write_tsv(impo, paste0("seir/data/important_", state, "_", format(max_date, "%Y%m%d"), ".txt"))
write_tsv(impo, paste0("seir/data/important_", state,"_latest.txt"))
...
ggsave(
  filename = paste0("seir/plots/seirfansy_national_cases_latest.pdf"),
  plot = case_plot,
...
ggsave(
  filename = paste0("seir/plots/seirfansy_national_deaths_latest.pdf"),
  plot = death_plot,
  width = 7, height = 5, device = cairo_pdf)
...
ggsave(
  filename = paste0("seir/plots/seirfansy_national_cases_bar_latest.pdf"),
  plot = case_bar_plot,
  width = 7, height = 5, device = cairo_pdf)
...
ggsave(
  filename = paste0("seir/plots/seirfansy_national_deaths_bar_latest.pdf"),
  plot = death_bar_plot,
  width = 7, height = 5, device = cairo_pdf)
