...
for (i in 1:4) {
  tmp_a <- table_deaths[[i + 3]]
  tmp_a <- c(rep("-", i), tmp_a[(i+1):length(tmp_a)])
  table_deaths[[i+3]] <- tmp_a
}

if (mh == TRUE) {
  case_out  <- here("lockdown", "rupam", "table_mh_cases.txt")
  death_out <- here("lockdown", "rupam", "table_mh_deaths.txt")
} else if (kl == TRUE) {
  case_out  <- here("lockdown", "rupam", "table_kl_cases.txt")
  death_out <- here("lockdown", "rupam", "table_kl_deaths.txt")
} else {
  case_out  <- here("lockdown", "rupam", "table_cases.txt")
  death_out <- here("lockdown", "rupam", "table_deaths.txt")
}
