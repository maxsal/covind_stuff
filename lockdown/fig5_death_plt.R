...
mh <- FALSE
tmp_outname <- "fig5_death_plot.pdf"
# kl <- TRUE
...
clean_prep <- function(x, var) {
  ...
  # mar_01 <- obs %>% clean_scenario_cfr(p = p, use_cfr = cfr_t7, stop_obs = "2021-03-01", scen = "March 1")
  ...
  # apr_30 <- obs %>% clean_scenario_cfr(p = p, use_cfr = cfr_t7, stop_obs = "2021-04-30", scen = "April 30")
...
full_plt <- patched +
  plot_annotation(
    title    = "Predicted number of daily COVID-19 deaths under India lockdown schedule",
...
# save output ----------
ggsave(filename = here("lockdown", "fig", tmp_outname),
       plot     = full_plt,
       height   = 12,
       width    = 10,
