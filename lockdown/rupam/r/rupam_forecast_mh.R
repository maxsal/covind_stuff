...
g <- list.files("~/projects/covind_wave2/rupam/src")
for (i in seq_along(g)) {source(glue("~/projects/covind_wave2/rupam/src/{g[i]}"))}
...
if (arrayid == 1) {
  ...
  casename   <- glue("{last_obs + 1}_mh_smooth{span}")
  # pie        <- bake_pi(today = today, start_proj = start_proj, last_proj = last_proj, span = span)
...
if (arrayid == 6) {
  ...
  casename   <- glue("no_intervention_mh_smooth{span}")
