...
us <- us_dat %>%
  ...
  labs(
    ...
    x       = "Date",
    y       = glue("{n_lag}-day percent change"),
    caption = "**Source:** JHU CSSE GitHub (cases); The COVID Tracking Project (tests)<br>**\uA9 COV-IND-19 Study Group**"
  ) +
  scale_y_continuous(labels = scales::percent) +
  theme_classic() +
  theme(
...
uk_dat <- owid %>%
  ...
  filter(
    date >= "2020-12-01" & date <= uk_end
  )

uk_med_case_change <- uk_dat %>% pull(cases_change) %>% median()
uk_med_test_change <- uk_dat %>% pull(tests_change) %>% median()
...
uk <- uk_dat %>%
  ...
  labs(
    ...
    subtitle = glue("{format(as.Date('2020-12-01'), '%B %e, %Y')} to {format(uk_end, '%B %e, %Y')}"),
    x       = "Date",
    y       = glue("{n_lag}-day percent change"),
...
png(filename = here("fig", glue("uk_v_us_v_india_comp_plot.png")),
    width = 10, height = 18, units = "in", res = 320)
grid::grid.newpage()
grid::grid.draw(rbind(gA, gB, gC))
dev.off()

cairo_pdf(filename = here("fig", glue("uk_v_us_v_india_comp_plot.pdf")),
    width = 10, height = 18)
grid::grid.newpage()
grid::grid.draw(rbind(gA, gB, gC))
dev.off()
...
india_max_tpr      <- d %>% dplyr::filter(date >= (today - 30)) %>% filter(daily_tpr == max(daily_tpr)) %>% pull(daily_tpr)
india_max_tpr_date <- d %>% dplyr::filter(date >= (today - 30)) %>% filter(daily_tpr == max(daily_tpr)) %>% pull(date)
...
india2 <- d %>%
  ...
  annotate("segment", x = india_max_tpr_date, xend = india_max_tpr_date, y = india_max_tpr / coeff_d, yend = 0.15 / coeff_d) +
  annotate("label", label = glue("Max TPR, {trimws(format(india_max_tpr_date, '%m/%e'))}: {round(india_max_tpr*100, 1)}%"),
           x = india_max_tpr_date, y = 0.15 / coeff_d, alpha = 0.8) +
...
coeff_us <- .25 / us_dat %>% pull(daily_tests) %>% max(., na.rm = T)
tpr_us_mag <- us_dat %>% filter(date == "2021-01-30") %>% pull(daily_tpr)

us_max_tpr      <- us_dat %>% filter(daily_tpr == max(daily_tpr)) %>% pull(daily_tpr)
us_max_tpr_date <- us_dat %>% filter(daily_tpr == max(daily_tpr)) %>% pull(date)

us2 <- us_dat %>%
  ggplot(aes(x = date)) +
  geom_bar(aes(y = daily_tests), stat = "identity", fill = "#3C3B6E", alpha = 0.5) +
  geom_bar(aes(y = daily_cases), stat = "identity", fill = "#B22234") +
  geom_line(aes(y = daily_tpr / coeff_us), size = 1) +
  annotate("segment", x = as.Date("2021-01-30"), xend = as.Date("2021-01-30"), y = tpr_us_mag / coeff_us, yend = 0.125 / coeff_us) +
  annotate("label", label = "Test-positive rate",
           x = as.Date("2021-01-30"), y = 0.13 / coeff_us, alpha = 0.8) +
  
  annotate("segment", x = us_max_tpr_date, xend = us_max_tpr_date, y = us_max_tpr / coeff_us, yend = 0.05 / coeff_us) +
  annotate("label", label = glue("Max TPR, {trimws(format(us_max_tpr_date, '%m/%e'))}: {round(us_max_tpr*100, 1)}%"),
           x = us_max_tpr_date, y = 0.05 / coeff_us, alpha = 0.8) +
  ...
  scale_y_continuous(labels = scales::comma, sec.axis = sec_axis(~.*coeff_us, name = "Test-positive rate", labels = scales::percent)) +
  theme_classic() +
  theme(
    text = element_text(family = "Lato"),
    plot.title = element_markdown(size = 18, face = "bold"),
...
coeff_uk <- .25 / uk_dat %>% pull(daily_tests) %>% max(., na.rm = T)
tpr_uk_mag <- uk_dat %>% filter(date == "2021-01-30") %>% pull(daily_tpr)

uk_max_tpr      <- uk_dat %>% filter(daily_tpr == max(daily_tpr)) %>% pull(daily_tpr)
uk_max_tpr_date <- uk_dat %>% filter(daily_tpr == max(daily_tpr)) %>% pull(date)

uk2 <- uk_dat %>%
  ggplot(aes(x = date)) +
  geom_bar(aes(y = daily_tests), stat = "identity", fill = "#012169", alpha = 0.5) +
  geom_bar(aes(y = daily_cases), stat = "identity", fill = "#C8102E") +
  geom_line(aes(y = daily_tpr / coeff_uk), size = 1) +
  annotate("segment", x = as.Date("2021-01-30"), xend = as.Date("2021-01-30"), y = tpr_uk_mag / coeff_uk, yend = 0.125 / coeff_uk) +
  annotate("label", label = "Test-positive rate",
           x = as.Date("2021-01-30"), y = 0.13 / coeff_uk, alpha = 0.8) +
  
  annotate("segment", x = uk_max_tpr_date, xend = uk_max_tpr_date, y = uk_max_tpr / coeff_uk, yend = 0.20 / coeff_uk) +
  annotate("label", label = glue("Max TPR, {trimws(format(uk_max_tpr_date, '%m/%e'))}: {round(uk_max_tpr*100, 1)}%"),
           x = uk_max_tpr_date, y = 0.20 / coeff_uk, alpha = 0.8) +
  
  labs(
    title   = "COVID-19: <b style='color:#C8102E'>Daily cases</b> and <b style='color:#012169'>daily tests</b> in the UK",
    subtitle = glue("{format(as.Date('2020-11-01'), '%B %e, %Y')} to {format(us_end, '%B %e, %Y')}"),
    x       = "Date",
    y       = glue("Daily count"),
    caption = "**Source:** Our World in Data<br>**\uA9 COV-IND-19 Study Group**"
  ) +
  scale_y_continuous(labels = scales::comma, sec.axis = sec_axis(~.*coeff_uk, name = "Test-positive rate", labels = scales::percent)) +
  theme_classic() +
  theme(
    text = element_text(family = "Lato"),
    plot.title = element_markdown(size = 18, face = "bold"),
...
png(filename = here("fig", glue("uk_v_us_v_india_bar_plot.png")),
    width = 10, height = 18, units = "in", res = 320)
grid::grid.newpage()
grid::grid.draw(rbind(gC, gD, gE))
dev.off()

cairo_pdf(filename = here("fig", glue("uk_v_us_v_india_bar_plot.pdf")),
          width = 10, height = 18)
grid::grid.newpage()
grid::grid.draw(rbind(gC, gD, gE))
dev.off()
