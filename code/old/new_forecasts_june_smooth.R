...
if (arrayid == 1) {
  ...
  casename   <- glue("{last_obs + 1}_june_smooth{span}")
  pie        <- bake_pi(pi_start = "2020-06-03", today = today, start_proj = start_proj, last_proj = last_proj, span = span)
...
if (arrayid == 5) {
  ...
  message("lockdown start date: April 25, 2021")
  last_obs   <- as.Date("2021-04-24")
  start_obs  <- last_obs - 99
  start_proj <- last_obs + 1
  last_proj  <- last_obs + 60
  proj_days  <- as.numeric(last_proj - start_proj) - 1
...
if (arrayid == 6) {
  ...
  casename   <- glue("no_intervention_june_smooth{span}")
