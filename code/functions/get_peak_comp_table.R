...
get_peak_comp_table <- function(
  location         = "India",
  wave_2_start     = as.Date("2021-02-15"),
  state_start_date = as.Date("2020-04-15"),
  today            = Sys.Date() - 1,
  n_lag            = 30) {

  d <- try(read_csv(glue("https://raw.githubusercontent.com/umich-cphds/cov-ind-19-data/master/{today}/everything.csv"),
                       col_types = cols())
              )
  
  if(inherits(d, "try-error")) {
    stop("Cannot pull data. Check data repo and input latest date in `today` argument")
  }
  
    d <- d %>%
      dplyr::filter(place == location) %>%
      mutate(
        daily_tpr = daily_cases / daily_tests
      )
    
    if(location != "India") {
      d <- d %>% dplyr::filter(date >= state_start_date)
      d[is.na(d)] <- 1
    }
    ...
    w1b <- w1 %>%
      dplyr::filter(date >= as.Date("2020-06-03"))
    ...
    if (location != "India") {
      names(tib)[names(tib) == "tmp1"] <- glue("{format(state_start_date, '%B %e, %Y')} - February 14, 2021")
    } else {
      names(tib)[names(tib) == "tmp1"] <- glue("March 24, 2020 - February 14, 2021")
    }
    ...
    tib %>%
      ...
      # title
      tab_header(
        title    = md(glue("**Comparing COVID-19 Waves in {location}**")),
